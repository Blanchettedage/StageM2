...
import_multi<- function(chemin){
  
  files<- list.files(chemin)
  files
  ...
  for (i in 1:nb_files) {
              print(assign(data_names[[i]], 
                     read.csv2(file.path(chemin, files[i]) ) , envir= .GlobalEnv)
    )  
...
df<-import_multi("P41_2020_2021")


Blanka<-ls()

######### FONCTION DE CONNEXION A LA BASE DE DONEES   ############
Connexion_db <- function (User, mot_passe, host_db='localhost', port_db=5432, dbname){
...
con<- Connexion_db(User='postgres', mot_passe='dage', dbname='daphne_dev')


####### *************** IMPORTATION DES TABLES DE LA BASE DE DONNEES *********#########


import<- function(con, table_db, champs ){
...
# Taxo
taxo1<-import(con, "public.taxo", c("taxo_id", "taxo_code"))


#wday
ws1<-import(con, "public.ws", c("wscode", "wsname"))
ws1

# exp_unit
exp_unit_db<- import(con, "public.exp_unit" , c("unit_code", "exp_unit_id", "trial_code" ))
exp_unit_db

# Accession
accession1<-import(con, "public.accession",c("accession_code","accession_id" ))

# lot 
lot1<- import(con, "public.lot", c("lot_id", "lot_code"))
lot1

#factor_level
factor_level1<- import(con, "public.factor_level", c("factor_level_id", " factor_level" ))
factor_level1
...
Blanka<- c("itk.csv", "ws.csv", " accession.csv")
trait_sauve(Blanka)
